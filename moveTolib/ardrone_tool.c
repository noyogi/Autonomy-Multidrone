...
C_RESULT ardrone_tool_init( const char* ardrone_ip, size_t n, char* topic, AT_CODEC_FUNCTIONS_PTRS *ptrs, const char *appname, const char *usrname, const char *rootdir, const char *flightdir, int flight_storing_size, academy_download_new_media academy_download_new_media_func)
{
	C_RESULT res;
   ...
   if( ptrs != NULL )
    ...
    // Init custom tool
    res = ardrone_tool_init_custom(topic);
...
int ardrone_tool_main(int argc, char **argv)
{
  ...
  while (-1 == getDroneVersion (root_dir, wifi_ardrone_ip, &ardroneVersion))
	...
	res = ardrone_tool_setup_com(topic);
	...
	else
	{
		...
		*/
		...
		ardrone_gen_appid (appname, __SDK_VERSION__, app_id, app_name, sizeof (app_name));
		res = ardrone_tool_init(wifi_ardrone_ip, strlen(wifi_ardrone_ip),topic, NULL, appname, NULL, NULL, NULL, MAX_FLIGHT_STORING_SIZE, NULL);
...
#ifndef _WIN32
	C_RESULT ardrone_tool_init_custom(char* topic) { return C_OK; }
